...
def commodity_pipeline():
    ...
    def process_series(commodity: str):
        ...
        with requests.Session() as session:
            series, obs = get_data(
                commodity, session=session, observation_start=start_date
            )
            metadata: SeriesMetaData = SeriesMetaData.from_FRED_response(
                commodity, series
            )
            observations: SeriesObservations = SeriesObservations.from_FRED_response(
                commodity, obs
            )
            validate_series(
                observations, metadata, last_observation_date, last_observation_value
            )
