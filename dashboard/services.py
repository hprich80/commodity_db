...
def calculate_open_positions(
    ...
    for series in trades_by_series:
        ...
        for trade in trades_by_series[series]:
            ...
            closing_position = (
                True
                if (trade_quantity * net_quantity) < 0
                and (abs(trade_quantity) > abs(net_quantity))
                else False
            )
            ...
            if inverse_trade:
                ...
                trade_realised_pnl *= (
                    -1 if (net_quantity < 0 and not closing_position) else 1
                )
                realised_pnl += trade_realised_pnl
                if closing_position:
                    avg_cost = trade.price
            else:
                avg_cost = (avg_cost * net_quantity + trade.price * trade_quantity) / (
        ...
        position = Position(
            series, net_quantity, avg_cost, current_price, unrealised_pnl, realised_pnl
        )
...
def get_price_summary():
    ...
    for series_id, observations in prices.items():
        ...
        latest_price, pcnt_change, last_date = calculate_pcnt_change(
