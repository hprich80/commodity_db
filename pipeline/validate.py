...
def check_nulls(obs: SeriesObservations, max_null_threshold: int = 2):
    ...
    if (null_count := sum(1 for v in obs.value[-3:] if v is None)) > max_null_threshold:
        ...
        logger.info(
            f"Recent null rows ({null_count}) below threshold for series {obs.series_id}"
        )
...
def check_performance(
    ...
    def evaluate_pair(prev: float | None, cur: float | None, date_str: date | None):
        ...
        if (change := abs((cur - prev) / prev)) > max_perf_threshold:
            logger.warning(
                f"Performance of {change*100}% for series {obs.series_id} at {date_str}"
            )
            count += 1
    ...
    if count > 0:
        logger.warning(
            f"{count} rows with unusual performance for series {obs.series_id}"
        )
    else:
...
def check_staleness(
    ...
    if not last_observation_date:
        logger.warning(
            f"No last observation to check staleness for {metadata.series_id}"
        )
        return
    if (datetime.date.today() - last_observation_date).days > max_age:
        logger.warning(
            f"Series {obs.series_id} is potentially stale. Last record  was {last_observation_date} (frequency: {metadata.frequency})"
        )
    else:
        logger.info(
            f"Series {obs.series_id} (frequency: {metadata.frequency}) up to date"
        )
