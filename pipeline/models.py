...
class SeriesMetaData(Series):
    ...
    def from_FRED_response(
        ...
        if not (seriess_list := json.get("seriess")):
            logger.error(
                f"FRED response for {series_id} contains no series metadata. Response: {json}"
            )
            raise KeyError(
                f"FRED response for {series_id} contains no series metadata."
            )
        ...
        try:
            ...
            logger.error(
                f"Missing expected metadata for key {e} in FRED response for {series_id}"
            )
            raise
...
class SeriesObservations(Series):
    ...
    def from_FRED_response(
        ...
        if (observations := json.get("observations")) is None:
            logger.error(
                f"FRED response for {series_id} contains no series observations. Response: {json}"
            )
            raise KeyError(
                f"FRED response for {series_id} contains no series observations."
            )
        if not observations:
        ...
        try:
            ...
            logger.error(
                f"Failed to parse observations for {series_id}. Contains malformed value: {e}"
            )
            raise

        logger.info(
            f"Successfully parsed {len(datelist)} observations for {series_id}. Rows with missing values: {valuelist.count(None)}"
        )
        return cls(series_id=series_id, date=datelist, value=valuelist)
